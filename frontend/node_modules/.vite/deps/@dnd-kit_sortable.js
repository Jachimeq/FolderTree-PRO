...
function SortableContext(_ref) {
  ...
  const items = (0, import_react2.useMemo)(() => userDefinedItems.map((item) => typeof item === "object" && "id" in item ? item.id : item), [userDefinedItems]);
  const isDragging = active != null;
  const activeIndex = active ? items.indexOf(active.id) : -1;
  const overIndex = over ? items.indexOf(over.id) : -1;
  const previousItemsRef = (0, import_react2.useRef)(items);
...
function useDerivedTransform(_ref) {
  ...
  const [derivedTransform, setDerivedtransform] = (0, import_react2.useState)(null);
  const previousIndex = (0, import_react2.useRef)(index);
  useIsomorphicLayoutEffect(() => {
...
function useSortable(_ref) {
  ...
  const itemsAfterCurrentSortable = (0, import_react2.useMemo)(() => items.slice(items.indexOf(id)), [items, id]);
  const {
    rect,
    node,
    isOver,
